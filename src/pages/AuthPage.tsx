...
import { Mail, Lock, Eye, EyeOff, User, Phone, ArrowLeft, Loader2, KeyRound } from 'lucide-react';
...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      const msg = error === 'Invalid login credentials'
        ...
        ? 'Email chưa xác nhận. Hãy kiểm tra hộp thư.'
        : error;
      toast.error(msg);
      setLoading(false);
    } else {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPwd) { toast.error('Hai mật khẩu không khớp'); return; }
    if (!phone.match(/^0\d{9,10}$/)) { toast.error('Số điện thoại không hợp lệ (vd: 0912345678)'); return; }
    if (loading) return;
    setLoading(true);
    const { error, linkedOrders } = await signUp(email, password, { full_name: name, phone, name });
    if (error) {
    ...
    if (linkedOrders && linkedOrders > 0) {
      toast.success(`✅ Đăng ký thành công! Đã tìm thấy ${linkedOrders} đơn cũ của bạn.`, { duration: 5000 });
    } else {
      toast.success('Đăng ký thành công!');
    }
    // Try auto-login (in case email confirmation disabled)
  ...
  const handleForgot = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) { toast.error('Nhập email của bạn'); return; }
    setLoading(true);
    const { error } = await resetPassword(email);
    setLoading(false);
    if (error) toast.error('Lỗi: ' + error);
    else {
      toast.success('✅ Đã gửi link đặt lại mật khẩu! Kiểm tra hộp thư của bạn.', { duration: 5000 });
      setMode('login');
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    if (error) toast.error('Lỗi đăng nhập Google: ' + error);
          ...
          <div className="bg-card rounded-2xl border border-border p-6 shadow-lg">
            <div className="text-center mb-5">
              <h1 className="text-xl font-black text-foreground">
                {mode === 'login' ? 'Đăng nhập' : mode === 'signup' ? 'Đăng ký thành viên' : 'Quên mật khẩu'}
              </h1>
              <p className="text-xs text-muted-foreground mt-1">
                {mode === 'login' ? 'Chào mừng quay lại GIANG NGUYÊN SEAFOOD'
                  : mode === 'signup' ? 'Tạo tài khoản để tích điểm & nhận ưu đãi'
            ...
            {mode === 'signup' && (
              <form onSubmit={handleSignup} className="space-y-3">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
                      placeholder="0912345678" required pattern="0\d{9,10}"
                      className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/40" />
                  </div>
                  <p className="text-[10px] text-muted-foreground mt-1">Dùng để đồng bộ đơn hàng đã đặt trước đó.</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Mật khẩu *</label>
                  <div className="relative">
                ...
                <p className="text-[11px] text-muted-foreground text-center">
                  Bằng việc đăng ký, bạn đồng ý với <Link to="/chinh-sach" className="text-primary hover:underline">điều khoản</Link>.
                </p>
              </form>
            )}
            ...
            {mode === 'forgot' && (
              <form onSubmit={handleForgot} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Email đã đăng ký</label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
