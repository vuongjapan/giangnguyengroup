...
import { ArrowLeft, ShoppingCart, Minus, Plus, ChevronLeft, ChevronRight, ShieldCheck, Truck, RotateCcw, CheckCircle2, AlertTriangle, Snowflake, Users, Package, Phone, RefreshCw, ZoomIn, Star, Heart, Share2 } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  const tabs = [
    ...
    { id: 'cooking' as const, label: '🍳 Cách chế biến' },
      ...
      <main className="container mx-auto px-3 md:px-4 py-4 md:py-6 flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10">
          ...
          <div className="space-y-2 md:space-y-3">
            <div
              className="relative aspect-square rounded-xl overflow-hidden bg-muted cursor-zoom-in border border-border group"
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`h-3.5 w-3.5 ${i < product.rating ? 'fill-accent text-accent' : 'text-muted'}`} />
                  ))}
                </div>
                <span className="text-xs text-muted-foreground">({product.rating}/5)</span>
                <span className="text-xs text-muted-foreground">|</span>
                <span className="text-xs text-primary font-medium">Đã bán 200+</span>
              </div>
            </div>
          ...
          <div className="flex border-b border-border overflow-x-auto scrollbar-hide">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex-shrink-0 px-4 py-3 text-sm font-bold transition-colors relative whitespace-nowrap ${
                    ...
                    <table className="w-full text-sm">
                      <tbody>
                        {[
                          ...
                          ['Xuất xứ', desc.specs.origin],
                          ['Quy cách', desc.specs.weight],
                          ['Hạn sử dụng', desc.specs.expiry],
                          ['Đơn vị', product.unit],
            ...
            {activeTab === 'reviews' && (
              <div className="text-center py-12">
                <div className="flex justify-center gap-1 mb-3">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`h-6 w-6 ${i < product.rating ? 'fill-accent text-accent' : 'text-muted'}`} />
                  ))}
                </div>
                <p className="text-lg font-bold text-foreground">{product.rating}/5 sao</p>
                <p className="text-sm text-muted-foreground mt-1">Dựa trên đánh giá của khách hàng</p>
                <div className="mt-6 space-y-3 max-w-lg mx-auto text-left">
                  {[
                    { name: 'Chị Hương – Hà Nội', text: 'Hàng ngon, đóng gói cẩn thận, ship nhanh. Sẽ mua lại!', rating: 5 },
                    { name: 'Anh Minh – Sài Gòn', text: 'Mua lần thứ 3, lần nào cũng hài lòng. Chất lượng ổn định.', rating: 5 },
                    { name: 'Chị Lan – Đà Nẵng', text: 'Giá hợp lý, hải sản tươi ngon. Recommend cho mọi người.', rating: 4 },
                  ].map((review, i) => (
                    <div key={i} className="bg-secondary/40 rounded-xl p-4">
                      ...
                      <p className="text-sm text-foreground mb-1">"{review.text}"</p>
